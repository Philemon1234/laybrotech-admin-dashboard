...
import { FiBold, FiImage, FiItalic, FiLink, FiList, FiMinus, FiMoreHorizontal, FiRotateCcw, FiRotateCw, FiTable, FiType, FiUnderline } from 'react-icons/fi';
...
export function RichTextEditor({ value, onChange, postId }: RichTextEditorProps) {
  ...
  const editor = useEditor({
    extensions: [StarterKit.configure({ heading: { levels: [2, 3, 4] } }), Underline, Link.configure({ openOnClick: false, autolink: true, HTMLAttributes: { rel: 'noopener noreferrer', target: '_blank' } }), Image.configure({ HTMLAttributes: { class: 'blog-content-image' } }), Table.configure({ resizable: true, HTMLAttributes: { class: 'blog-content-table' } }), TableRow, TableHeader, TableCell, Placeholder.configure({ placeholder: 'Write the article body...' })],
    content: value || '',
  ...
  return (
    <div>
      <BubbleMenu editor={editor} options={{ placement: 'top', offset: 8, shift: true }} shouldShow={({ editor }) => editor.isFocused && !editor.state.selection.empty}>
      ...
      <div className="sticky top-16 z-30 flex flex-nowrap items-center gap-1 overflow-x-auto border border-brand-border bg-white px-3 py-2">
